...
from .serializers import RolPermisoSerializaer
...
# CREAR Y LISTAR LAS RELACIONES ENTRE ROL Y PERMISOS
# *******************************************************************************************
@api_view(['GET','POST'])
@authentication_classes([TokenAuthentication])
@permission_classes([IsAuthenticated])
def roles_permisos(request):
    if request.method == 'GET':
        if not request.user.is_staff:
            return Response({"error":"No autorizado"}, status=status.HTTP_401_UNAUTHORIZED)
        roles_permisos = Rol_Permiso.objects.all().order_by('id')
        serializer = RolPermisoSerializaer(roles_permisos, many=True)
        return Response({"msg":"Se ha listado correctamente","data":serializer.data}, status=status.HTTP_200_OK)
    elif request.method == 'POST':
        if not request.user.is_staff:
            return Response({"error":"No autorizado"}, status=status.HTTP_401_UNAUTHORIZED)
        serializer = RolPermisoSerializaer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response({"msg":"Relación entre rol y permiso creado correctamente","data":serializer.data}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
